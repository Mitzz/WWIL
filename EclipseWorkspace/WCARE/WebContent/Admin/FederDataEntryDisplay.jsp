...
function get_page() 
{
	 ...
	 req.send("Admin_Input_Type=findFDPaged&AppId="+ApplicationId);
}
function showData(dataXml,a,b)
{
	var cart = dataXml.getElementsByTagName("fdpagemaster")[0];
	...
	for (var I = 0 ; I < items.length ; I++)
   	{	  	
     	...
     	if (nname != null)
     	{
     		...
     		{     		
	   			str+="&nbsp;<a href='FederEntryForm.jsp?siteid=<%=siteid %>&date=<%=date %>&type=<%=type %>&strt="+item.getElementsByTagName("pagestart")[0].firstChild.nodeValue+"&endd="+item.getElementsByTagName("pageend")[0].firstChild.nodeValue+"' target='Fdiframe'>" + (I + 1) + "</a>&nbsp;";
	   		}
   	 	}
	}		
...
<table>
		<tr>
			<td colspan="2" align="left"><input type="button"  class="printbutton"  style="color:white;font=bold;background-color:green" value="Previous"  onClick=location.href="FederDataEntryDisplay.jsp?siteid=<%=siteid %>&date=<%=prevdate %>&type=<%=type %>&strt=1&endd=5"></TD>
			<td colspan="4" align="center"><b><font size="4">Reading Date:<%=date %></font></b></td>
			<td colspan="2"  align="right"><input type="button"  class="printbutton"  style="color:white;font=bold;background-color:green" value="Next" onClick=location.href="FederDataEntryDisplay.jsp?siteid=<%=siteid %>&date=<%=nextdate %>&type=<%=type %>&strt=1&endd=5"></TD>
   		</TR>
   	</table>
<iframe align="middle" id="Fdiframe" name="Fdiframe" width="100%" height="650" 
style="BORDER-RIGHT:0px; BORDER-TOP:0px; BORDER-LEFT:0px; BORDER-BOTTOM:0px"
frameborder="0" src="FederEntryForm.jsp?siteid=<%=siteid %>&date=<%=date %>&type=<%=type %>&strt=1&endd=5"></iframe>		
