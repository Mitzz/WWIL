...
function findSearch() 
{
	var blnSave=false;
     ...
     if ( blnSave == true ) {
		...
		if (blnSave == true)
		{
    		window.open("FederDataEntryDisplay.jsp?siteid="+document.getElementById("SiteIdtxt").value+"&date="+document.getElementById("Datetxt").value+"&type="+document.getElementById("Typetxt").value,'FederEntryForm','height=725,width=1250,top=0,resizable=no,scrollbars=yes,menubar=no,toolbar=no,status=no');
    	}
     } else {
       return false;
     }
...
function showAppDetails(dataXml)
{
	...
	var items = cart.getElementsByTagName("fdcode");	
		...
		str +="<tr align='center' height='20'><th class='detailsheading' width='30'>S.N.</th><th class='detailsheading' width='200'>Feder</th><th class='detailsheading' width='100'>Short Description</th>"
		str +="<th class='detailsheading' width='30'>Deactivate</th></tr>";
	   	...
	   	{	  	
	     	...
	     	if (nname != null){
	     		...
	     		//if (item.getElementsByTagName("fdstatus")[0].firstChild.nodeValue == 1){
		     		...
		     		//str+="onclick=updateFDStatus('" + item.getElementsByTagName("fdid")[0].firstChild.nodeValue+"',"
...
function updateFDStatus(fdid,chkbox) 
{
	 var req = newXMLHttpRequest();
     var ApplicationId = document.getElementById(chkbox.id).checked + "," + fdid;
	 req.onreadystatechange = getReadyStateHandler(req, UpdateFDStat);	  
	 req.open("POST", "<%=request.getContextPath()%>/Ajax.do", true);
	 req.setRequestHeader("Content-Type", "application/x-www-form-urlencoded");
	 req.send("Admin_Input_Type=SU_UpdateFDStat&AppId="+ApplicationId);
}
function UpdateFDStat(dataXml)
{
...
<form action="<%=request.getContextPath()%>/FDParameter.do" method="post" name="FDParameter">
<table align="center" border="0" cellpadding="0" cellspacing="0" width="500">
	<tbody><tr>
	<td class="newhead1"></td>
	<th class="headtext">Feeder Reading</th>
	...
	<table border="0" cellpadding="0" cellspacing="0" width="100%"><tbody><tr><td bgcolor="#dbeaf5">
		...
		<tr class="bgcolor"> 
			...
			<td class="bgcolor">
				...
				<a href="javascript:void(0)" id="dc" onClick="if(self.gfPop)gfPop.fPopCalendar(document.FDParameter.Datetxt);return false;" ><img class="PopcalTrigger" align="absmiddle" src="<%=request.getContextPath()%>/resources/images/calbtn.gif" border="0" alt=""></a>
			</td>
		</tr>			
		<tr class="bgcolor"> 
