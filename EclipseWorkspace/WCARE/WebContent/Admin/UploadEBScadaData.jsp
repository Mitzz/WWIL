...
function displayScadaEBDataToUpload()
{	
	 ...
	 req.send("Admin_Input_Type=getScadaEBDataToUpload&AppId="+ApplicationId);
}
function showAppDetails(dataXml)
{
	...
	var items = cart.getElementsByTagName("stateDataCode");
		...
		str +="<th class='detailsheading' width='120'>Reading Date</th><th class='detailsheading' width='120'>Total EB To Publish</th><th class='detailsheading' width='40'>Display Scada Data</th><th class='detailsheading' width='50'>Publish</th></tr>";
		for (var I = 0 ; I < items.length ; I++)
	   	{	  	
	     	...
	     	if (nname != null){
	     		...
	     		if (item.getElementsByTagName("ebtoupload")[0].firstChild.nodeValue == ".")
	     		{
	     			str+="<td align='left'></td>"
	     		}
	     		else
	     		{
	     			str+="<td align='center'>" + item.getElementsByTagName("ebtoupload")[0].firstChild.nodeValue + "</td>"
	     		}		     		
...
<form action="<%=request.getContextPath()%>/UploadEBScadaData.do" method="post" name="UploadEBScadaData" onSubmit="return findvalidate()">
...
<tr>
	...
	<tbody><tr>
		<td class="btn" width="100">
			<input type="button" name="Submitcmd" class="btnform" value="Generate" onClick="displayScadaEBDataToUpload()"/>
			<input type="hidden" name="Admin_Input_Type" value="UploadEBScadaData" />
		</td>
