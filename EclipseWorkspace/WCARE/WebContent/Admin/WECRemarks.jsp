...
{    
     ...
     var ApplicationId = list.options[list.selectedIndex].value+','+document.forms[0].Datetxt.value+','+document.forms[0].Datetxt2.value;
	 alert(ApplicationId);
	 req.onreadystatechange = getReadyStateHandler(req, showAppDetails);	  
	 req.open("POST", "<%=request.getContextPath()%>/Ajax.do", true);
	 req.setRequestHeader("Content-Type", "application/x-www-form-urlencoded");
	 req.send("Admin_Input_Type=findWECRemarks&AppId="+ApplicationId);
}
function showAppDetails(dataXml)
{
	...
	var items = cart.getElementsByTagName("weccode");	
		...
		str +="<tr align='center' height='20'><th class='detailsheading' width='30'>S.N.</th><th class='detailsheading' width='150'>WEC Description</th><th class='detailsheading' width='80'>Remarks ID.</th>"
		str +="<th class='detailsheading' width='30'>Remarks Desc</th><th class='detailsheading' width='30'>Staus</th></tr>";
	   	...
	   	{	  	
	     	...
	     	if (nname != null){
			    ...
			    if (item.getElementsByTagName("desc")[0].firstChild.nodeValue == ".")
		     	...
		     	if (item.getElementsByTagName("remarksid")[0].firstChild.nodeValue == ".")
		     		str+="<td align='left'></td>"	  
		     	else
		     		str+="<td align='left'>" + item.getElementsByTagName("remarksid")[0].firstChild.nodeValue + "</td>"
		     	if (item.getElementsByTagName("remarksdesc")[0].firstChild.nodeValue == ".")
		     		str+="<td align='left'></td>"	  
		     	else
		     		str+="<td align='left'>" + item.getElementsByTagName("remarksdesc")[0].firstChild.nodeValue + "</td>"
...
<table align="center" border="0" cellpadding="0" cellspacing="0" width="500">
	...
	<th class="headtext">WEC Remarks</th>
...
<tr>
	...
	<table border="0" cellpadding="0" cellspacing="0" width="100%"><tbody><tr><td bgcolor="#dbeaf5">
		...
		<tr class="bgcolor"> 
			...
			<td class="bgcolor">
				<input type="text" name="Datetxt2" id="Datetxt2" size="20" class="ctrl" maxlength="10" onfocus="dc.focus()" />
				<a href="javascript:void(0)" id="dc" onClick="if(self.gfPop)gfPop.fPopCalendar(document.EBParameter.Datetxt2);return false;" ><img class="PopcalTrigger" align="absmiddle" src="<%=request.getContextPath()%>/resources/images/calbtn.gif" border="0" alt=""></a>
			</td>
		</tr>	
		<tr class="bgcolor"> 
			<td colspan="2">
...
<tr>
	...
	<tbody><tr>
		<td class="btn" width="100">
			<input type="button" name="Searchcmd" class="btnform" value="Submit" onclick="findApplication()" />
		</td>
