...
//	String Customeridtxt = "";
        ...
        function myFunction(ur, wty, wna, eb, wid, rd, state, cname, type, fiscalYear){
            var url = ur + "?wectype=" + wty + "&wecname=" + wna + "&ebid=" + eb + "&wecid=" + wid + "&rd=" + rd + "&state=" + state + "&cname=" + cname + "&type=" + type + "&year=" + fiscalYear;
            window.open(url, 'name', 'height=600,width=800, top=100,resizable=yes,scrollbars=yes,menubar=no,toolbar=no,status=no');
        }
        </script>
    </HEAD>
...
int fiscalYear = Integer.parseInt(request.getParameter("year"));

//System.out.println("In CustomerReportByYear_WECWise.jsp Year:" + fiscalYear);
if(debug){
...
if (type.equals("Y")) {
                            %>
                            <input type="button" value="View All Machine" class="printbutton" onClick=location.href = "AllMachineReportY.jsp?custid=<%=custid%>&rd=<%=rdate%>&siteid=<%=siteid%>&stateid=<%=stateid%>">
                            <%
} 
                            %>
                            <input type="button" value="Print" onClick="window.print()" class="printbutton">
                            <input align="right" type="button" class="printbutton" value="Excel" onClick=location.href="../ExportCustomerReport.jsp?stateid=<%=stateid%>&siteid=<%=siteid %>&id=<%=custid%>&type=<%=type%>&rd=<%=rdate%>&year=<%=fiscalYear%>">
							<input align="right" type="button" class="printbutton" value="PDF" onClick=location.href="../ExportCustomerReportToPDF.jsp?stateid=<%=stateid%>&siteid=<%=siteid %>&id=<%=custid%>&type=<%=type%>&rd=<%=rdate%>&year=<%=fiscalYear%>"></td>
                    </TR>
                    <TR>
                        <td colspan="6"></td>
                    </tr>
...
for (int i = 0; i < getEBHeadingList_1.size(); i++) {
	...
	getWECDataList_3 = (List) CustomerUtility.getOneEBWECWiseTotalForOneFiscalYear(ebid, fiscalYear);
	if(debug){
	...
	if (wecsize > 0) {
		...
		for (int j = 0; j < getWECDataList_3.size(); j++) {
			...
			if (type.equals("Y")) {
                                        ...
                                        <TD class=TableCell1 align="left">
                                        	<a href="javascript:onClick=myFunction('CustomerReportYByYear_WECWise.jsp','<%=getWECDataListEach_3_4.get(1)%>','<%=name%>','<%=ebid%>','<%=getWECDataListEach_3_4.get(8)%>','<%=rdate%>','<%=state%>','<%=cname%>','Y','<%=fiscalYear%>')"><%=getWECDataListEach_3_4.get(0)%></a>
										</TD>
                                        <%
			}
                                        %>
        ...
        if (wecsize > 1) {
			List getEBWiseTotalList_5 = (List) CustomerUtility.getOneEBTotalForOneFiscalYear(ebid, fiscalYear);
			if(debug){
