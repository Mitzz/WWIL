...
//System.out.println("Month In CustomerReportMByMonth_Machine:" + monthReceived);
...
<TABLE cellSpacing=0 cellPadding=0 width="100%" border=0>
  ...
  <TR align="left">
     <TD  align="left">
       ...
       <input  align="right" type="button" class="printbutton"  value="Excel" onClick="location.href='../ExportCustomerM.jsp?wectype=<%=wectype%>&wecname=<%=wecname%>&ebid=<%=ebid%>&wecid=<%=wecid%>&rd=<%=rdate%>&state=<%=state%>&cname=<%=cname%>&year=<%=yearReceived%>&month=<%=monthReceived%>&type=M'";/>
     </td>
      </TR>  
        <TR>  
  ...
  <TR>
    <TD>
      <TABLE cellSpacing=1 cellPadding=2 width="100%" border=0>
       ...
       tranListData = (List)CustomerUtility.getOneWECInfoForOneMonth(wecid, monthReceived, yearReceived);  
       cls="TableRow1";
   ...
   for (int j=0; j <tranListData.size(); j++){   
        ...
        <%
         	tranListData.clear();
         tranListData = CustomerUtility.getOneWECTotalForOneMonth(wecid, monthReceived, yearReceived);   
         for (int j=0; j <tranListData.size(); j++){
         	Vector vdata = new Vector();
         	vdata = (Vector)tranListData.get(j);
         	//String name = (String)vdata.get(0);
