...
//System.out.println("In CustomerReportYByYear_WECWise Year:" + fiscalYear);
...
<TABLE cellSpacing=0 cellPadding=0 width="90%" border=0>
  <TBODY>
   <TR align="left">
     ...
     <input  align="right" type="button" value="Excel" class="printbutton" onClick="location.href='../ExportCustomerY.jsp?wectype=<%=wectype%>&wecname=<%=wecname%>&ebid=<%=ebid%>&wecid=<%=wecid%>&rd=<%=rdate%>&state=<%=state%>&cname=<%=cname%>&type=Y&year=<%=fiscalYear%>'"; />
     </TD>
       </TR> 
  <TR>
  ...
  <TR>
    <TD>
      <TABLE cellSpacing=1 cellPadding=2 width="100%" border=0>
       ...
       <%
                         ...
                         tranListData = (List)CustomerUtility.getOneWECMonthWiseTotalForOneFiscalYear(wecid, fiscalYear);  
   ...
   <%
	for (int j=0; j <tranListData.size(); j++){   
        ...
        <% 	
	tranListData.clear();
    tranListData = (List)CustomerUtility.getOneWECTotalForOneFiscalYear(wecid, fiscalYear);   
    for (int j=0; j <tranListData.size(); j++){
		Vector vdata = new Vector();
		vdata = (Vector)tranListData.get(j);
			//String name = (String)vdata.get(0);
