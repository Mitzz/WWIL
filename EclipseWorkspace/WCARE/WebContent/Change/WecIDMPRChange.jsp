...
if(file==null || file.equals("")) file="WecIDMPRChange.xls";
...
<%
	...
	if(debug){
		...
		System.out.println("wecid:" + wecid);
	}
	List<Object> wecTotal = CustomerUtility.getOneWECTotalForBetweenDaysBasedOnWECId(wecid, DateUtility.convertDateFormats(fdate, "dd/MM/yyyy", "dd-MMM-yyy"), DateUtility.convertDateFormats(tdate, "dd/MM/yyyy", "dd-MMM-yyy"));
	List<String> customerMetaInfo = CustomerUtility.getCustomerMetaInfoBasedOnWECId(wecid);
	if(debug){
		GlobalUtils.displayVectorMember(wecTotal);
	}
	...
	<TBODY>
        ...
        <TR bgcolor="#CCCCCC">
                     ...
                     <td vAlign ="middle"  align = "center" colspan="7"><b><%=customerMetaInfo.get(0)%></b></td><!-- Customer Name -->
		               <%   
		               if(wecTotal.size() == 2 || wecTotal.size() == 1){
		            	 %>
					<td vAlign = "middle" align = "center" colspan="2"><b>Capacity : <%="NA"%></b></td><!--  Load Capacity -->
		</TR>
		<TR>		             
					...
					<td vAlign = "middle" align = "center" colspan = 13><%=wecTotal.get(1)%></td>
		            	 ...
		            	 <%  
		            	   ...
		            	   %>
		            	                        <td vAlign = "middle" align = "center" colspan="2"><b>Capacity : <%=loadCapacity%></b></td><!--  Load Capacity -->
		</TR>
		            	   <TR>		             
