...
if (type.equals("Y")) {
                            ...
                            <input align="right" type="button" class="printbutton" value="Excel" onClick=location.href="ExportCustomerReport.jsp?stateid=<%=stateid%>&siteid=<%=siteid %>&id=<%=custid%>&type=<%=type%>&rd=<%=rdate%>">
							<input align="right" type="button" class="printbutton" value="PDF" onClick=location.href="ExportCustomerReportToPDF.jsp?stateid=<%=stateid%>&siteid=<%=siteid %>&id=<%=custid%>&type=<%=type%>&rd=<%=rdate%>"></td>
                    </TR>
                    <TR>
                        <td colspan="6"></td>
                    </tr>
...
if(type.equals("D")){
			...
			<TABLE cellSpacing=0 class=SectionTitle1 cellPadding=0 width="90%" border=0>
				...
				<tr>
					<td colspan="2" align="left"><input type="button" class="printbutton"  style="color:white;font=bold;background-color:green" value="Previous"  onClick=location.href="CustomerReport.jsp?id=<%=custid%>&rd=<%=prevdate%>&siteid=<%=siteid%>&stateid=<%=stateid%>&type=<%=type%>"></TD>
					<td colspan="4" align="center"></td>
					<td colspan="2"  align="right"><input type="button"  class="printbutton"  style="color:white;font=bold;background-color:green" value="Next" onClick=location.href="CustomerReport.jsp?id=<%=custid%>&rd=<%=nextDate%>&siteid=<%=siteid%>&stateid=<%=stateid%>&type=<%=type%>"></TD>
	   			</TR>
			</TABLE>
			<%
}
		...
		for (int j = 0; j < getWECDataList_3.size(); j++) {
			...
			if (type.equals("D")) {
                                        ...
                                        <TD class=TableCell1 align="left">
                                        	<a href="javascript:onClick=myFunction('CustomerReportM.jsp','<%=getWECDataListEach_3_4.get(1)%>','<%=name%>','<%=ebid%>','<%=getWECDataListEach_3_4.get(8)%>','<%=rdate%>','<%=state%>','<%=cname%>','M')"><%=getWECDataListEach_3_4.get(0)%></a>
										</TD>
                                        <%
			}
			else if (type.equals("Y")) {
