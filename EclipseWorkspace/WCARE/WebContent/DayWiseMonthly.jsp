...
/* MethodClass.displayMethodClassName();
GlobalUtils.displayRequestParameter(request); */
String atype = "DM";
	%>
	<TABLE cellSpacing=0 cellPadding=0 width="90%" border=0>
		<TBODY>
			<TR>
				<TD class=SectionTitle1 colspan="3" noWrap><input type="button" value="Back" onClick=location.href='DGRView.jsp?id=<%=custid%>&rd=<%=rdate%>&type=<%=atype%>' class="printbutton">
				</TD>
				<TD class=SectionTitle1 colspan="4" align="right"><input type="button" value="Print" onClick="window.print()" class="printbutton">
				<input type="button" value="Excel" onClick=location.href='ExportDayWise.jsp?id=<%=custid%>&rd=<%=rdate%>&type=<%=type%>&stateid=<%=stateid%>&siteid=<%=siteid%>' class="printbutton">
				</TD>
			</TR>
		</TBODY>
	</TABLE>
...
<%
	...
	//GlobalUtils.displayVectorMember(tranListData);
	int tsize = 0;
	cls = "TableRow1";
				...
				if (j != 0) {
					nedate = ad.convertDateFormat(pdate, "dd-MM-yyyy", "dd/MM/yyyy");
			 		...
			 		if (tsize > 0) {
			 			...
			 			//GlobalUtils.displayVectorMember(wecListData);
					...
					//GlobalUtils.displayVectorMember(ebListData);
					cls = "TableRow1";
