		...
		<TR>
			...
			if(!wecid.equals("ALL")){%>
				...
				<td align="center" colspan="2"> 
					...
					<%-- <input  align="right" type="button" value="Export To Excel" onClick=location.href="mprWecType.do?wectype=<%=wec%>&fdate=<%=fdate%>&tdate=<%=tdate%>&ebid=<%=ebid%>&wecid=<%=wecid%>"> --%>
					<input  align="right" type="button" value="Export To Excel" onClick=location.href="mprWecType.do?fdate=<%=fdate%>&tdate=<%=tdate%>&wecid=<%=wecid%>">
			...
			<%-- One Particular Eb is selected with all Wec is selected --%>
				...
				<td align="center" colspan="2"> <input  align="right" type="button" value="Export To Excel" onClick=location.href="mprWecType.do?fdate=<%=fdate%>&tdate=<%=tdate%>&ebid=<%=ebid%>"></td>
					
			<% }else if(state.equals("ALL")){%>
				<%-- Only Wec Type Select --%>
				<!-- System.out.println("All State"); -->
				<td align="center" colspan="2"> <input  align="right" type="button" value="Export To Excel" onClick=location.href="mprWecType.do?wectype=<%=wec%>&fdate=<%=fdate%>&tdate=<%=tdate%>"></td>
			...
			<% }else if(siteid.equals("ALL")){%>
				...
				<td align="center" colspan="2">
					 ...
					 <input  align="right" type="button" value="Export To Excel" onClick=location.href="mprWecType.do?stateid=<%=state%>&wectype=<%=wec%>&fdate=<%=fdate%>&tdate=<%=tdate%>">
				</td>
			<%} else{%>
				<!-- System.out.println(); -->
				...
				<td align="center" colspan="2"> 
					<input  align="right" type="button" value="Export To Excel" onClick=location.href="mprWecType.do?stateid=<%=state%>&wectype=<%=wec%>&fdate=<%=fdate%>&tdate=<%=tdate%>&siteid=<%=siteid%>">
				</td>
			<%}%>
		</TR>
		<TR>
