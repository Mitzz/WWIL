...
function findApplication() 
{
     ...
     var ApplicationId = document.forms[0].logIdmastertxt.value;
	 req.onreadystatechange = getReadyStateHandler(req, showAppDetails);	  
	 req.open("POST", "<%=request.getContextPath()%>/Ajax.do", true);
	 req.setRequestHeader("Content-Type", "application/x-www-form-urlencoded");
	 req.send("Admin_Input_Type=CUST_LOGINDETAIL_ID&AppId="+ApplicationId);
}
		...
		for (var I = 0 ; I < items.length ; I++)
	   	{   	 
	     	...
	     	{   
	     		...
	     		document.forms[0].CustNametxt2.value =item.getElementsByTagName("scustname")[0].firstChild.nodeValue;
...
<%
			...
			lid =request.getParameter("uid");
			
			//cname=request.getParameter("name").toString();
