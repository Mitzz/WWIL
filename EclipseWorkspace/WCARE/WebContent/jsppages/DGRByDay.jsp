	            ...
	            <div id='menu' align='left'>
		            ...
		            <Table  cellSpacing=0 cellPadding=0 width=100% bgColor=#555555 border=1>
						...
						<tr class=${ class }>
							...
							<td class=TableCell1>
								<%-- <a href="<c:url value="/Change/CustomerReportChangeByDay.jsp"/>?stateid=<c:out value="${ siteDetail.state.id }" />&siteid=<c:out value="${ siteDetail.site.id }" />&id=<c:out value="${ siteDetail.customer.id }"/>&rd=<c:out value="${ reportDate }" />" target="_blank"> ${ siteDetail.site.name }</a> --%>
								<a href="<c:url value="/dgrWec.do"/>?stateId=<c:out value="${ siteDetail.state.id }" />&siteId=<c:out value="${ siteDetail.site.id }" />&customerId=<c:out value="${ siteDetail.customer.id }"/>&date=<c:out value="${ reportDate }" />" target="_blank"> ${ siteDetail.site.name }</a>
							</td>
