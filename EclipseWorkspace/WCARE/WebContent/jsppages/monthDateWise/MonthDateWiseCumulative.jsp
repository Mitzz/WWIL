	            ...
	            <div id='menu' align='left'>
		            ...
		            <Table  cellSpacing=0 cellPadding=0 width=100% bgColor=#555555 border=1>
						...
						<tr class=${ class }>
							...
							<td class=TableCell1>
								<a href="<c:url value="/monthDateWiseDetail.do"/>?stateId=<c:out value="${ siteDetail.state.id }" />&siteId=<c:out value="${ siteDetail.site.id }" />&customerId=<c:out value="${ siteDetail.customer.id }"/>&fromDate=<c:out value="${ fromDate }" />&toDate=<c:out value="${ toDate }" />" target="_blank"> ${ siteDetail.site.name }</a>
							</td>
