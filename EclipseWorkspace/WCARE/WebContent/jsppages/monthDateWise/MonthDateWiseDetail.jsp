    ...
    <BODY text=#000000 bottomMargin=0 bgColor=#ffffff leftMargin=0 topMargin=0 rightMargin=0 marginheight="0" marginwidth="0">
        <CENTER>
            <TABLE cellSpacing=0 class=SectionTitle1 cellPadding=0 width="90%" border=0>
                ...
                <tr>
                	<TD class=SectionTitle1 colspan="3" noWrap><input type="button" value="Back" onClick=location.href="<c:url value="monthDateWiseCumulative.do"/>?customerId=<c:out value="${ customerId }" />&month=<c:out value="${ month }" />&year=<c:out value="${ year }" />">
				</TD>
				<TD class=SectionTitle1 colspan="4" align="right"><input type="button" value="Print" onClick="window.print()" class="printbutton">
				<input type="button" value="Excel" onClick=location.href="<c:url value="ExportDayWise.jsp"/>?id=<c:out value="${ customerId }" />&rd=<c:out value="${ firstDate }" />&type=<c:out value="M" />&stateid=<c:out value="${ stateId }" />&siteid=<c:out value="${ siteId }" />" class="printbutton">
            ...
            <TABLE cellSpacing=0 cellPadding=0 width="90%" border=0>
                <TBODY>
                    <TR>
                        <TD class=SectionTitle colspan="6" align="center" noWrap>Monthly Date Wise Report</TD>
                    </TR>	
                </TBODY>
            </TABLE>
            <c:choose>
            <c:when test="${ dataPresent }">
            <c:forEach items="${ ebsMonthWise }" var="ebMonthWise">
            
            <table cellSpacing=0 cellPadding=0 width="90%" border=0>
                <tbody>
                    <tr class=TableTitle>
                        <td valign=middle class=TableCell align=center width=90% colspan="6">${ ebMonthWise.customer.name }</td>
                    </tr>
                    ...
                    <tr class=TableSummaryRow>
                        ...
                        <td valign=middle align=left width=38% colspan=2>${ ebMonthWise.eb.site.name } - ${ ebMonthWise.eb.state.name }</td>

                        <td valign=middle class=TableCell1 align=left width=18%>Machines:</td>
                        <td valign=middle align=left width=24% colspan=2>${ ebMonthWise.typeCount } </td>
                    ...
                    <tr class=TableSummaryRow>
                        ...
                        <td valign=middle align=left width=38% colspan=2>${ ebMonthWise.capacity/1000 } MW</td>
            ...
            <c:forEach items="${ebMonthWise.view}" var="ebDgr" varStatus="ebDgrCount">
            <table cellSpacing=0 cellPadding=0 width="90%" border=0>
                <tbody>
                    <tr>
                        <td class=SectionTitle colspan="6" align="center" noWrap>${ ebDgr.date }</td>
                    </tr>
                </tbody>
            </table>
            ...
            <table cellSpacing=0 cellPadding=0 width="90%" border=0>
                <tbody>
                    <tr>
                        <td class=SectionTitle colspan="6" align="center" noWrap>EB Data</td>
                    </tr>
                </tbody>
            </table>
            <table cellSpacing=0 cellPadding=0 width="90%" bgColor=#555555 border=0>
