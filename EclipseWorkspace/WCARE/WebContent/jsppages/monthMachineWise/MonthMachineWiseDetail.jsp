...
%>
        ...
        <script type="text/javascript">
        	//May be not called 
            function myFunction(ur, wecId, month, year) {
                var url = ur + "?wecId=" + wecId + "&month=" + month + "&year=" + year;
                ...
                <TBODY>
                    <tr>
                        <TD align="right" colspan="6">
                        	<input align="right" type="button" class="printbutton" value="View All Machine" onClick="location.href='<c:url value="monthMachineWiseAllMachine.do"/>?stateId=<c:out value="${ stateId }" />&siteId=<c:out value="${ siteId }" />&customerId=<c:out value="${ customerId }" />&month=<c:out value="${ month }" />&year=<c:out value="${ year }" />'" />
							...
							<input align="right" type="button" class="printbutton" value="Excel" onClick="location.href='<c:url value="ExportCustomerReport.jsp"/>?stateid=<c:out value="${ stateId }" />&siteid=<c:out value="${ siteId }" />&id=<c:out value="${ customerId }" />&type=M&rd=<c:out value="${ firstDate }" />'" />
							<input align="right" type="button" class="printbutton" value="PDF" onClick="location.href='<c:url value="ExportCustomerReportToPDF.jsp"/>?stateid=<c:out value="${ stateId }" />&siteid=<c:out value="${ siteId }" />&id=<c:out value="${ customerId }" />&type=M&rd=<c:out value="${ firstDate }" />'" />
						</td>
                    </TR>
                    <TR>
                        <td colspan="6"></td>
                                ...
                                <tbody>
                                    ...
                                    <tr class=${counter % 2 == 0 ? "TableRow2" : "TableRow1"}>
                                   	...
                                   	<trial:isTrial wec="${wecData.key}" var="inTrial" ></trial:isTrial>
                                        ...
                                        <td class=TableCell1 align="left"><a href="javascript:onClick=myFunction('<c:url value="/wecDateWise.do"/>','${wecData.key.id}','${ month }','${ year }')">${wecData.key.name} (${ wecData.key.foundationNo })</a></td>
                                   	<c:choose>
                                   	<c:when test="${show}">
