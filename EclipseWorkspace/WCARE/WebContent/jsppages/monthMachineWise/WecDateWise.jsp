			  		...
			  		<TR align="left">		 
				        <TD  align="left">
				       		 <input type="button" value="Print" onClick="window.print()" class="printbutton">
				       		  ...
				       		  <input align="right" type="button"  value="Excel" onClick="location.href='<c:url value="ExportCustomerM.jsp"/>?wectype=<c:out value="${ wecDateWise.wec.type }/${ wecDateWise.wec.capacity }" />&wecname=<c:out value="${ wecDateWise.wec.name }" />&ebid=<c:out value="${ wecDateWise.wec.eb.id }" />&wecid=<c:out value="${ wecDateWise.wec.id }" />&rd=<c:out value="${ reportDate }" />&state=<c:out value="${wecDateWise.wec.site.name }-${wecDateWise.wec.state.name }" />&cname=<c:out value="${wecDateWise.wec.customer.name }" />&type=M'" />  
