...
%>
        ...
        <script type="text/javascript">
        	function myFunction(ur, wecId, fiscalYear) {
                var url = ur + "?wecId=" + wecId + "&fiscalYear=" + fiscalYear;
                ...
                <TBODY>
                    <tr>
                        <TD align="right" colspan="6">
                        	<input align="right" type="button" class="printbutton" value="View All Machine" onClick="location.href='<c:url value="yearWecWiseAllMachine.do"/>?stateId=<c:out value="${ stateId }" />&siteId=<c:out value="${ siteId }" />&customerId=<c:out value="${ customerId }" />&fiscalYear=<c:out value="${ fiscalYear }" />'" />
							...
							<input align="right" type="button" class="printbutton" value="Excel" onClick="location.href='<c:url value="ExportCustomerReport.jsp"/>?stateid=<c:out value="${ stateId }" />&siteid=<c:out value="${ siteId }" />&id=<c:out value="${ customerId }" />&type=Y&rd=<c:out value="${ firstDate }" />'" />
							<input align="right" type="button" class="printbutton" value="PDF" onClick="location.href='<c:url value="ExportCustomerReportToPDF.jsp"/>?stateid=<c:out value="${ stateId }" />&siteid=<c:out value="${ siteId }" />&id=<c:out value="${ customerId }" />&type=Y&rd=<c:out value="${ firstDate }" />'" />
						</td>
                    </TR>
                    <TR>
                        <td colspan="6"></td>
            ...
            <table cellSpacing=0 cellPadding=0 width="90%" border=0>
                <tbody>
                    ...
                    <tr class=TableSummaryRow>
                        ...
                        <td valign=middle class=TableCell1 align=left width=18%>FY:</td>
                                ...
                                <tbody>
                                    ...
                                    <tr class=${counter % 2 == 0 ? "TableRow2" : "TableRow1"}>
                                   	...
                                   	<trial:isTrial wec="${wecData.key}" var="inTrial" ></trial:isTrial>
                                        ...
                                        <td class=TableCell1 align="left"><a href="javascript:onClick=myFunction('<c:url value="/wecMonthWise.do"/>','${wecData.key.id}','${ fiscalYear }')">${wecData.key.name} (${ wecData.key.foundationNo })</a></td>
                                   	<c:choose>
                                   	<c:when test="${show}">
