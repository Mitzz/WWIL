...
import com.enercon.struts.form.GridBifurcationDataUploadForm;

public class GridBifurcationDataUploadAction extends Action{
	private final static Logger logger = Logger.getLogger(GridBifurcationDataUploadAction.class);
	public GridBifurcationDataUploadAction() {}
...
//		MethodClass.displayMethodClassName();
		...
		GridBifurcationDataUploadForm gridUpload = null;
		WcareDao dao = new WcareDao();
		
		try{
			gridUpload = (GridBifurcationDataUploadForm) form;
			
			FormFile excelFile = gridUpload.getExcelFile();
			File uploadedExcelFile = uploadToServer(excelFile);
			
			ArrayList<ArrayList<String>> excelData = readExcelData(uploadedExcelFile, gridUpload.getExcelSheetName());

			dao.validateData(excelData);
			...
			logger.error("\nClass: " + e.getClass() + "\nMessage: " + e.getMessage() + "\n");
			messages.add("gridBifurcationException", new ActionMessage(e.getMessage(), e.getRecordNo()));
			saveMessages(request, messages); // storing messages as request attributes
		}
